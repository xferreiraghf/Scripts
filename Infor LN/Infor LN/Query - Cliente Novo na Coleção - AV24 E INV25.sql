SELECT
    ...
    CASE
        WHEN EXISTS (
            ...
            WHERE ov_ant.t$ofbp = pn.t$bpid  
              AND ov_ant.t$orno <> ov.t$orno  
              AND lp1.t$cprj$c <> '3.2024'  
              AND ov_ant.t$stov$c IN ('1', '2', '3', '4', '5', '6', '7', '8')  
              AND ov_ant.t$stov$c NOT IN ('9', '10')  
              AND ov_ant.t$styp$c IN ('002', '102')  
              AND ov_ant.t$cpay <> '172'  
        ) THEN 'Recorrente'
        WHEN EXISTS (
            SELECT 1
            FROM igrppr.ttdsls400001 ov_ant
...
FROM
    ...
    JOIN igrppr.ttdsls400001 ov ON pn.t$bpid = ov.t$ofbp  
    JOIN igrppr.ttdzbi401001 ri ON ov.t$orno = ri.t$orno$c  
    JOIN igrppr.ttcmcs034001 lp ON ov.t$cpls = lp.t$cplt  
    JOIN igrppr.ttcmcs052001 pj ON lp.t$cprj$c = pj.t$cprj  
    JOIN igrppr.ttdsls012001 dv ON ov.t$cofc = dv.t$cofc  

WHERE
    pn.t$ctit = '001'       
    AND ov.t$stov$c NOT IN ('9', '10')   
    AND ov.t$styp$c IN ('002', '102')    
    AND ov.t$cpay <> '172'     
    AND lp.t$cprj$c = '3.2024'  
    ...
    CASE
        WHEN EXISTS (
            ...
            WHERE ov_ant.t$ofbp = pn.t$bpid  /* Mesmo cliente */
              ...
              AND lp1.t$cprj$c <> '5.2025'  /* Coleção diferente de '5.2025' */
        ...
        WHEN EXISTS (
            ...
            WHERE ov_ant.t$ofbp = pn.t$bpid  /* Mesmo cliente */
              AND ov_ant.t$orno <> ov.t$orno  /* Pedidos diferentes */
              AND lp1.t$cprj$c = '5.2025'  /* Coleção '2.2024' */
...
WHERE
    ...
    AND lp.t$cprj$c = '5.2025'  /* Coleção '3.2024' (Alto Verão 2024) */
...
HAVING
    CASE
        WHEN EXISTS (
            ...
            WHERE ov_ant.t$ofbp = pn.t$bpid  /* Mesmo cliente */
              AND ov_ant.t$orno <> ov.t$orno  /* Pedidos diferentes */
              AND lp1.t$cprj$c <> '5.2025'  /* Coleção diferente de '3.2024' */
              AND ov_ant.t$stov$c IN ('1', '2', '3', '4', '5', '6', '7', '8')  /* Situações de ordem recorrente */
              AND ov_ant.t$stov$c NOT IN ('9', '10')  /* Não considerar Cancelados e Devolvidos */
              AND ov_ant.t$styp$c IN ('002', '102')  /* Apenas segmentos 002 e 102 */
              AND ov_ant.t$cpay <> '172'  /* Condição de pagamento válida */
        ...
        WHEN EXISTS (
            ...
            WHERE ov_ant.t$ofbp = pn.t$bpid  /* Mesmo cliente */
              AND ov_ant.t$orno <> ov.t$orno  /* Pedidos diferentes */
              AND lp1.t$cprj$c = '5.2025'  /* Coleção '3.2024' */
              AND ov_ant.t$stov$c IN ('9', '10')  /* Situações de ordem nova */
              AND ov_ant.t$styp$c IN ('002', '102')  /* Apenas segmentos 002 e 102 */
              AND ov_ant.t$cpay <> '172'  /* Condição de pagamento válida */
        ) THEN 'Novo'
