SELECT 
	...
	ads.t$ftyp$l AS "Tipo entidade",
	ads.t$fovn$l AS "Entidade",
	pnc.t$cbrn AS "Cod. segmento",
	decode (pnc.t$cbrn, 
	...
	ads.t$ccty AS "País",
	ads.t$cste AS "UF",
	ads.t$ccit AS "Cod. cidade",
	ads.t$dsca AS "Desc. cidade",
	ads.t$ddd1$c AS "DDD",
	ads.t$telp AS "Telefone",
	ads.t$info AS "E-mail principal",
	ads.t$enfe$l AS "E-mail NFE",
  ...
  WHEN pn.t$code$c = 34 THEN 'STATUS NÃO DEFINIDO'
 	...
 	cc.t$styp$C AS "Tipo de venda",
 	tp.t$desc AS "Descrição"
 	
FROM 
    igrppr.ttccom100001 pn
    JOIN igrppr.ttccom110001 pnc ON pn.t$bpid = pnc.t$ofbp
    JOIN igrppr.ttccom130001 ads ON pn.t$cadr = ads.t$cadr
    JOIN igrppr.ttdzbr003001 cc ON cc.t$bpid$c = pn.t$bpid
    JOIN igrppr.ttcmcs065001 dp ON dp.t$cwoc = cc.t$repr$c
    JOIN igrppr.ttcmcs202001 tp ON tp.t$styp = cc.t$styp$C
WHERE 
    pn.t$ctit = '001' /* Apenas clientes */
    AND cc.t$ativ$c = '1' /* Ativo */
    AND cc.t$styp$C IN ('002', '003', '102', '103') /* Tipos de venda */
    
/* By Ferreira */
    
/* Atualizado: 11/12/2024 | Inserção dos campos 'cc.t$styp$C' e 'tp.t$desc', relacionados ao tipo de venda. */ 
